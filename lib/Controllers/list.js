...
List.prototype.fetch = function(req, res, context) {
  ...
  if (_.has(req.query, sortParam) || _.has(this.resource.sort, 'default')) {
    ...
    sortColumns.forEach(function(sortColumn) {
      if (sortColumn.indexOf('-') === 0) {
        ...
        // Note: workaround for not working with camelCase model and snackCase column
  ...
  // query an extra row to check if there is more
  options.limit++;
  ...
  return find(options)
    .then(function(result) {
      var rows = options.skipCount ? result : result.rows,
          count = options.skipCount ? null : result.count;
      if (rows.length === parseInt(options.limit)) {
