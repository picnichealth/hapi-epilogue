...
var Resource = function(options) {
  _.defaults(options, {
    actions: ['create', 'read', 'update', 'delete', 'list', 'download', 'search', 'batchUpdate'],
    pagination: true,
  ...
  this.refetchInstances = options.refetchInstances;
    ...
    if (_.has(middleware, action)) {
      _.forOwn(middleware[action], function(definition, milestone) {
        if (_.isFunction(definition)) {
          if (milestone === 'error') {
            self.controllers[action].error = definition;
          } else {
...
function autoAssociate(resource) {
  ...
  _.forEach(resource.model.associations, function(association) {
    ...
    if (association.associationType === 'HasOne') {
      ...
      // Note for hapi: duplicate route is not allowed
      if (!resource[subResourceName]) resource[subResourceName] = hasOneResource(Resource, resource, association);
    } else if (association.associationType === 'HasMany') {
      subResourceName =
            association.target.options.name.plural.toLowerCase();
      if (!resource[subResourceName]) resource[subResourceName] = hasManyResource(Resource, resource, association);
    } else if (association.associationType === 'BelongsTo') {
      subResourceName =
        association.target.options.name.singular.toLowerCase();
      if (!resource[subResourceName]) resource[subResourceName] = belongsToResource(Resource, resource, association);
    } else if (association.associationType === 'BelongsToMany') {
      subResourceName =
        association.target.options.name.plural.toLowerCase();
      if (!resource[subResourceName]) resource[subResourceName] = belongsToManyResource(Resource, resource, association);
    }
  });
}
